...
async fn health_checker_handler() -> impl Responder {
    const MESSAGE: &str = "Simple REST API with Rust and Actix Web";
...
async fn create_todo_handler(
    ...
    if todo.is_some() {
        ...
        // Conflict() represents status 409
    ...
    // save todo item inside in-memory db
...
async fn get_todo_handler(path: web::Path<String>, data: web::Data<AppState>) -> impl Responder {
    ...
    // todo is an Option<>, save Some() into todo using unwrap.
    // shadowing
...
async fn edit_todo_handler(
    ...
    let payload = Todo {
        ...
        completed: if body.completed.is_some() {
            body.completed // where is unwrap?
        } else {
            todo.completed
        },
